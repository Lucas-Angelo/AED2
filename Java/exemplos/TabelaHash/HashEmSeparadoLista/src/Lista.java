public class Lista {
	...
	public void inserir(Inteiro elemento, int posicao) throws Exception {
		...
		if (posicao <= tamanho) {
			...
			throw new Exception("Erro ao tentar inserir na lista: posição inválida.");
		}
	...
	public Inteiro remover(int posicao) throws Exception {
		...
		if (!listaVazia()) {
			...
			if (posicao < tamanho) {
				...
				throw new Exception("Erro ao tentar remover da lista: posição inválida!");
			}
	...
	public Inteiro localizar(int chave) {
		Celula aux;

		aux = primeiro.proximo;
		while (aux != null) {

			if (aux.item.getDado() == chave)
				return aux.item;
			else
				aux = aux.proximo;
		}
